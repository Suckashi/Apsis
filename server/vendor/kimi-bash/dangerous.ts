// Adapted from MoonshotAI/kimi-code be7d5f5fea7800778e4660cd5f36780ba783bddd.
// See ./LICENSE and ./README.md. Analysis logic is unchanged.
import { parse, type ParseResult } from './parse.ts';
import type { SyntaxNode as BashSyntaxNode } from './node.ts';
type BashParseResult = Exclude<ParseResult, {ok: true}> | {ok: true; hasError: boolean; root: BashSyntaxNode};
...
export function analyzeDangerousCommand(command: string): DangerousVerdict | undefined {
  return analyzeSource(command, 0, (source) => {
    const result = parse(source, PARSE_OPTIONS);
    return result.ok ? { ...result, root: result.rootNode } : result;
  });
}
