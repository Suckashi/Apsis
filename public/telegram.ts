...
export function createTelegramUI(api: Api, notify: (text: string) => void) {
  ...
  function renderState(view: TelegramView) {
    ...
    $("#telegram-owner").textContent = view.ownerId
      ? "已綁定你的 Telegram 帳號 · " + view.ownerId
      : "尚未配對，只有配對後的帳號能使用工作區。";
    $("#telegram-error").textContent = view.error;
  ...
  $("#telegram-form").addEventListener("input", () => status("變更尚未儲存。"));
  ...
  $("#telegram-form").addEventListener("submit", (event) => {
    ...
    void action(async () => {
      ...
      loaded = false;
      $("#telegram-pairing").hidden = true;
      status("設定已儲存。啟用後等候連線成功，再產生配對碼。");
      notify("Bot 設定已儲存，不需重新啟動。");
    });
  });
  $("#telegram-test").addEventListener(
    ...
    () =>
      void action(async () => {
        ...
        status("已連接 @" + result.username + "，Token 有效。");
      }),
  );
  ...
  $("#telegram-unpair").addEventListener(
    ...
    () =>
      void action(async () => {
        await post("/unpair");
        status("已解除綁定並停止 bot 任務；歷史對話仍保留在 Web。");
      }),
  );
