...
按下**設定**齒輪，在彈出的視窗切換至**模型連線**，選擇服務、填入金鑰及一個或多個模型 ID，然後儲存。為一般 Apsis 對話指定預設連線與模型；開始新對話前，也可直接在聊天輸入區選擇模型。修改後不需重新啟動。尚未設定模型時，可在輸入框的 **任務選項**選擇 **示範模式**，體驗不呼叫 API 的預先編寫串流回覆。

開發指令會一起啟動伺服器與前端建置。修改檔案後，瀏覽器資源會重新編譯，後端則自動重啟；前端更新後請重新整理瀏覽器。

## 建立 Agent

開啟 **設定 → 我的 Agents → 建立 Agent**，可從研究、程式碼閱讀或寫作範本開始，再設定角色、命名模型連線、模型 ID、工具、技能與記憶範圍。執行引擎放在「進階設定」。儲存後按 **開始對話** 即可試聊，也可從輸入框的任務選項選擇 agent。
...
所有引擎都在 Node.js 執行，不需要 Python 服務、LangSmith 部署或資料庫伺服器。npm 依賴會增加，引擎模組則按需載入。模型需支援工具與串流。每個 agent 選擇命名模型連線及其中一個模型 ID；各連線獨立保存網址與金鑰。原有 Bot 設定仍供 Telegram 與舊版設定使用。
...
命名連線及其金鑰保存在 `.loom/connections.json`。Telegram 與既有設定使用的原有 Bot 設定保存在 `.loom/settings.json`，優先於環境變數；移除原有金鑰也會停用環境變數中的備援值。介面不會修改 `.env`；手動修改環境變數後，需要重新啟動伺服器。
...
- 按下**設定**齒輪會開啟同一個設定視窗，可在其中切換模型連線、我的 Agents、記憶、技能與 Bot 設定；**任務紀錄**仍是獨立頁面。
...
2. 在 **設定 → Bot 設定 → Telegram Bot** 填入 Token，啟用 Bot 並儲存。
