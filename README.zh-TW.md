...
Bot 可以向其他 Bot 派工或提問並取得結果。秘書 Bot 可依角色尋找合適的成員、分派任務，再回到原對話彙整回覆。「Bot 協作」卡片會顯示進度，並提供前往接收方核准操作的入口。範例與執行限制請見 [Bot 協作與秘書派工](docs/bot-workspace.md#bot-協作與秘書派工)。
...
- 同時保存多個模型服務及模型，供對話或 Agent 選用。
- 使用通訊軟體式介面，包含深色、淺色、系統外觀、手機導覽、歷史搜尋、Markdown 與程式碼複製。

Web 版面參考 [xAI 的 Grok Bot 官方設計](https://x.ai/news/designing-grok-bot)，保留 Apsis 自己的識別與功能。Web 介面支援繁體中文與英文，可在**設定 → 一般 → 介面語言**切換。切換會重新載入頁面，保留聊天草稿並將偏好記在目前瀏覽器；已保存的對話、專案名稱、程式碼與工具輸出保留原文。任務權限和一般重新整理時一樣會重設。專案文件也提供英文與繁體中文兩版。
...
按下**設定**齒輪，在彈出的視窗切換至**模型連線**，選擇服務、填入金鑰及一個或多個模型 ID，然後儲存。為一般 Apsis 對話指定預設連線與模型；也可在聊天輸入區切換模型，既有對話會保留訊息，下一次回覆使用新模型。修改後不需重新啟動。尚未設定模型時，首頁會引導新增服務。示範模式不再出現在一般操作中，舊示範對話仍可查看。

開發指令會一起啟動伺服器與前端建置。修改檔案後，瀏覽器資源會重新編譯，後端則自動重啟；前端更新後請重新整理瀏覽器。

## 建立 Agent

開啟 **設定 → 我的 Agents → 建立 Agent**，可從研究、程式碼閱讀或寫作範本開始，再設定角色、命名模型連線、模型 ID、工具、技能與記憶範圍。執行引擎放在「進階設定」。儲存後按 **開始對話** 即可試聊，也可從側欄切換 agent。
...
所有引擎都在 Node.js 執行，不需要 Python 服務、LangSmith 部署或資料庫伺服器。npm 依賴會增加，引擎模組則按需載入。模型需支援工具與串流。每個 agent 選擇命名模型連線及其中一個模型 ID；各連線獨立保存網址與金鑰。Web 與 Telegram 共用模型連線管理。
...
在 **模型連線**先選服務，再設定端點與金鑰，加入該服務提供的完整模型 ID，並指定此連線的預設模型。Agent 編輯器會依服務分組列出可用連線，供你選取連線與模型。可同時保存多個 OpenAI 相容服務，不必覆蓋原有設定。更換供應商或網址會清除舊金鑰，避免誤送到新端點。仍被 Agent 或既有對話使用的連線不可封存。

## 背景任務與操作紀錄

**專案選擇：**在輸入框上方選擇專案，或按「加入專案」，填入執行 Apsis 的電腦上的既有完整資料夾路徑。新對話會保存專案設定，檔案工具、Shell 與檔案面板使用同一位置。舊對話仍使用預設工作區；換專案請按「另開專案對話」。資料夾搬走或消失時會報錯，不會自動另建資料夾。專案選擇不是 Shell 沙箱，同專案的對話仍共用檔案。

**操作結果：**回覆下方「查看操作結果」與任務紀錄可展開工具輸出、`write_file`／`edit_file` 的修改差異，以及 Shell 命令與退出碼；失敗或停止也會保留已收到的輸出。每份輸出／差異／命令最多保留 24,000 字元，超過會標示截斷。透過 Shell 修改的檔案目前不會自動產生 diff。舊任務未保存的輸出無法補回。「回合結束」只表示執行結束，不等於所有要求都已驗證通過。

**中斷接續：**「檢查並接續」會準備可編輯的訊息，確認本次權限後再送出。失敗、取消或重啟中斷後的下一則訊息，都會向三種引擎帶入同對話、自上次回合正常結束以來的操作紀錄摘要（最多八次、16,000 字元），要求先核對現況再執行。不會自動重播命令，仍以送出時的本次權限為準；模型仍從上次成功保存的狀態接續。這提供可查證的線索，不保證模型一定完成所有剩餘步驟。

任務由伺服器持有。送出後即可直接切換對話或開啟設定；關閉分頁或網路斷線都不會停止任務。側欄「任務紀錄」提供狀態、回覆、工具名稱、目標、操作結果與模型回報的 token 用量，並以每頁 50 筆載入。可停止進行中的任務或開啟來源對話。
...
新增連線時，先從 OpenAI、Anthropic、Ollama，以及 Kimi、DeepSeek、OpenRouter、Qwen 的 OpenAI 相容預設選擇，也可自訂端點。預設範本只提供連線資訊，不代表新增原生協定。一個連線可保存多個模型 ID，並指定預設模型。一般 Apsis 對話可在聊天輸入區選擇或切換連線與模型，保留同一對話的既有訊息；自建 Agent 則使用自己的連線與模型。

| 模型服務                         | 需要設定                                  | 連接方式                                   |
| -------------------------------- | ----------------------------------------- | ------------------------------------------ |
| OpenAI                           | 模型 ID 與 OpenAI API key                 | Pi 的 OpenAI provider                      |
| Anthropic                        | 模型 ID 與 Anthropic API key              | Pi 的 Anthropic provider                   |
| Ollama                           | 本機回送 HTTP 網址與已安裝模型 ID         | OpenAI 相容 Chat Completions               |
| Kimi、DeepSeek、OpenRouter、Qwen | 預設 Base URL、完整模型 ID 與服務 API key | OpenAI 相容 Chat Completions               |
| 自訂 OpenAI 相容 API             | Base URL、模型 ID；免驗證服務可不填金鑰   | 支援 SSE 串流與函式工具的 Chat Completions |

### OpenAI 與 Anthropic

在 **模型連線**選擇 OpenAI 或 Anthropic，填入 API key、加入要使用的模型 ID 並儲存。兩種服務的金鑰會分別保存。選擇連線而未另指定模型時，會使用該連線的預設模型。

設定狀態代表已提供設定值。「模型連線 → 測試模型」會以指定引擎與模型實際驗證串流、工具呼叫與工具結果回傳；雲端模型可能計費。測試使用隔離狀態，不讀取工作區或已存知識。測試結果只代表該模型／引擎組合；修改設定會清除舊結果。
...
4. 點選 **讀取本機模型**，加入要使用的模型、指定預設模型並儲存。
...
儲存的命名連線可供 Web 對話與 Agent 使用。一般對話會保存目前選擇的連線與模型，手動切換後由下一次回覆使用新選擇；已在執行的任務保留開始時的設定。設定 API 不會回傳金鑰，密碼欄位也不會重新填入已儲存的金鑰。如需移除，請勾選移除金鑰選項後儲存。

命名連線及其金鑰保存在 `.loom/connections.json`。首次偵測到的舊設定（`.loom/settings.json` 優先於環境變數）會自動匯入模型連線，保留原識別碼，並綁定既有對話與 Agent。之後請統一在模型連線修改金鑰與端點；重啟不會覆蓋已匯入的修改，也不會重新啟用已封存的連線。介面不會修改 `.env`；環境變數只供尚未匯入的服務使用，修改後需要重新啟動伺服器。
...
- 按下**設定**齒輪會開啟同一個設定視窗，可在其中切換模型連線、我的 Agents、記憶、技能與 Telegram；**任務紀錄**仍是獨立頁面。
- Apsis 名稱旁的 **＋** 用於開啟新話題。已保存的記憶與技能會保留，第一則訊息送出時才建立對話。
- 輸入框內的 **＋** 開啟任務選項；檔案、記憶、技能的寫入權限分別勾選，預設全部關閉。
- 一般 Apsis 對話可在聊天輸入區選擇或切換已設定的連線與模型，保留原有對話；Agent 對話使用該 Agent 保存的模型。
- Agent 統一從側欄切換。模型診斷在服務卡片的「進階：測試模型」；記憶及技能的編輯、合併與修訂紀錄收在每筆「管理」中。
...
- 按頂部檔案按鈕可查看工作區檔案；桌機使用側面板，手機使用抽屜。
...
2. 在 **設定 → Telegram** 填入 Token，啟用 Bot 並儲存。
3. 連線成功後產生配對碼，將畫面上的 `/pair …` 指令私訊給 Bot。配對碼十分鐘後失效，且只能使用一次。
4. 傳送任務後，可在 Web 歷史紀錄看到該對話、查看進度、停止任務或繼續聊天。

Bot 的新預設對話使用 **模型連線**中選定的 Apsis 預設模型，並共用 Apsis 的工作區、記憶與技能。也能透過對話選單中的 Telegram 續聊指令，在私訊中繼續 Web 對話；續聊的對話會保留其原本選用的連線與模型。
...
| `.loom/connections.json` | 命名服務、模型 ID、預設值及其金鑰              |
