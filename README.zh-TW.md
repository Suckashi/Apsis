...
設定狀態代表已提供設定值，不代表已成功驗證金鑰或產生回覆。實際傳送聊天任務才會使用模型連線。
...
- 輸入框內的 **＋** 會開啟任務選項。修改檔案、保存記憶與技能前，需開啟 **允許修改與保存**。
...
- 草稿依對話儲存在目前瀏覽器。任務失敗時會還原輸入內容供修改，不會自動重新送出。
...
遠端登入取得的是**完整擁有者權限**，包含模型設定、對話與任務執行。所有應用程式頁面與 API 都需要登入。登入狀態使用 HttpOnly、Secure、SameSite Cookie，八小時後過期，也可透過遠端登出結束。代理會檢查指定主機名稱、請求來源，並限制登入失敗次數。模型回覆透過 fetch 與 NDJSON 串流傳至瀏覽器。
...
| 優先順序 | 改進項目                                 | 預期效益                                                           |
| -------- | ---------------------------------------- | ------------------------------------------------------------------ |
| 1        | 記憶去重、來源追蹤、修訂歷史與相關性檢索 | 減少重複或過時資訊，將上下文預算留給與當前任務相關的記憶。         |
| 2        | 依 token 計算的上下文預算與對話摘要      | 讓長對話更穩定，並適應不同模型的上下文限制。                       |
| 3        | 連線測試與模型能力設定                   | 在任務開始前驗證金鑰、串流與工具呼叫，並允許調整輸出和上下文限制。 |
| 4        | 任務檢查點、明確失敗原因與手動恢復／重試 | 中斷後能繼續處理，同時避免重複寫入檔案或執行其他有副作用的操作。   |
...
新增工具時，在 `server/agent.ts` 的 `createTools()` 定義輸入結構與執行內容，並回傳結構化工具結果。修改資料的操作需套用既有寫入權限檢查。模型金鑰與外部整合應由伺服器處理。
