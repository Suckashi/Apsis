...
按下**設定**齒輪，在彈出的視窗切換至**模型連線**，選擇服務、填入金鑰及一個或多個模型 ID，然後儲存。為一般 Apsis 對話指定預設連線與模型；開始新對話前，也可直接在聊天輸入區選擇模型。修改後不需重新啟動。尚未設定模型時，首頁會引導新增服務。示範模式不再出現在一般操作中，舊示範對話仍可查看。
...
新增連線時，先從 OpenAI、Anthropic、Ollama，以及 Kimi、DeepSeek、OpenRouter、Qwen 的 OpenAI 相容預設選擇，也可自訂端點。預設範本只提供連線資訊，不代表新增原生協定。一個連線可保存多個模型 ID，並指定預設模型。聊天輸入區可為新的一般 Apsis 對話選擇連線與模型；自建 Agent 則使用自己的連線與模型。既有對話續聊時沿用原先選擇。
...
儲存的命名連線可供新 Web 對話與 Agent 使用。連線與模型會綁定該對話，續聊既有對話時沿用原選擇；已在執行的任務保留開始時的設定。設定 API 不會回傳金鑰，密碼欄位也不會重新填入已儲存的金鑰。如需移除，請勾選移除金鑰選項後儲存。
...
- 新的一般 Apsis 對話可在聊天輸入區選擇已設定的連線與模型，再傳送第一則訊息。Agent 對話則使用該 Agent 保存的模型。
